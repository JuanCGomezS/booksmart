...
import { parseCancellationNote } from './cancellation-note.js';
...
type ProductRequest = { productId: string; name?: string; quantity: number };
type BookingRequest = {
  businessId: string;
  serviceId: string;
  bookingDate: string;
...
type PublicBusinessRequest = { slug: string };
...
type PublicThemeDto = {
  id: string;
  palette?: Record<'background' | 'surface' | 'text' | 'primary', string>;
};
const publicPresetThemeIds = new Set([
  'gold-night',
  'royal-night',
  'crimson-sun',
  'violet-blush',
...
function businessLocalToday(now = new Date()): string {
...
function businessLocalDateTime(date: string, time: string): Date | null {
  const clock = minutes(time);
  if (!validDate(date) || clock === null) return null;
...
function serviceBufferMinutes(value: unknown): number {
  return positiveInteger(value, 0, 0, 240);
}

/** Keeps the DTO and booking callable on the same safe legacy field contract. */
function bookingCustomerFields(value: unknown): BookingCustomerFields {
  const configured = value && typeof value === 'object' ? (value as Record<string, unknown>) : {};
  const field = (name: keyof BookingCustomerFields): BookingCustomerFieldState =>
    configured[name] === 'required' || configured[name] === 'optional'
      ? configured[name]
      : 'disabled';
...
function effectiveBookingSettings(value: unknown): EffectiveBookingSettings {
  const configured = value && typeof value === 'object' ? (value as Record<string, unknown>) : {};
  return {
    ...
    exceptionalClosures: Array.isArray(configured.exceptionalClosures)
      ? configured.exceptionalClosures
      : [],
...
function isBusinessOperational(
  business: DocumentData | undefined,
  now = new Date(),
): business is DocumentData {
  if (!business || business.active !== true) return false;
  ...
  if (
    business.subscriptionStartsAt !== undefined &&
    (!startsAt || !Number.isFinite(startsAt.getTime()) || startsAt > now)
  )
    return false;
  if (business.planExpiresAt === undefined) return true;

  const expiresAt = toDate(business.planExpiresAt);
  return Boolean(expiresAt && Number.isFinite(expiresAt.getTime()) && expiresAt >= now);
}

function publicUnavailable(): HttpsError {
  // Do not disclose whether a slug exists, is inactive, or has expired.
  return new HttpsError('not-found', 'Public business unavailable.');
}

function publicString(value: unknown, maximum = 2_048): string | undefined {
...
function publicThemeDto(value: unknown): PublicThemeDto {
  ...
  const palette =
    theme.palette && typeof theme.palette === 'object' && !Array.isArray(theme.palette)
  ...
  if (
    ...
    Object.keys(palette).length === 4 &&
    keys.every(
      (key) => typeof palette[key] === 'string' && hexColorPattern.test(palette[key] as string),
    )
  ) {
    return {
      id: 'custom',
      palette: Object.fromEntries(
        keys.map((key) => [key, palette[key]]),
      ) as PublicThemeDto['palette'],
    };
  }
  return { id: 'gold-night' };
}

function publicWorkingHours(
  value: unknown,
  ...
  return Object.fromEntries(
    Array.from({ length: 7 }, (_, day) => {
      ...
      const { open, close, enabled } = schedule as Record<string, unknown>;
...
type PublicClosureRule =
  | { id: string; kind: 'weekly'; weekday: number; startTime?: string; endTime?: string }
  | { id: string; kind: 'date'; date: string; startTime?: string; endTime?: string };

/** Normalizes canonical and legacy closures into the one public booking contract. */
function publicClosureRules(booking: EffectiveBookingSettings): PublicClosureRule[] {
  const rules: PublicClosureRule[] = [];
  ...
  for (const closure of booking.exceptionalClosures) {
    const date =
      closure && typeof closure === 'object'
        ? (closure as Record<string, unknown>).date
        : undefined;
    const id = typeof date === 'string' ? `legacy-${date}` : '';
    if (!validDate(date) || ids.has(id)) continue;
...
function publicStaffSchedule(value: unknown): PublicBusinessResponse['staff'][number]['schedule'] {
  if (!value || typeof value !== 'object') return undefined;
  const source = value as Record<string, unknown>;
  return Object.fromEntries(
    Array.from({ length: 7 }, (_, day) => {
      const rawDay = source[String(day)];
      if (rawDay === undefined) return [day, undefined];
      if (!rawDay || typeof rawDay !== 'object')
        return [day, { enabled: false, start: '09:00', end: '18:00', breaks: [] }];
      const schedule = rawDay as Record<string, unknown>;
      const start =
        typeof schedule.start === 'string' && timePattern.test(schedule.start)
          ? schedule.start
          : undefined;
      const end =
        typeof schedule.end === 'string' && timePattern.test(schedule.end)
          ? schedule.end
          : undefined;
      ...
      const invalidBreak =
        ...
        schedule.breaks.some((item) => {
          if (!item || typeof item !== 'object') return true;
          const range = item as Record<string, unknown>;
          return (
            typeof range.start !== 'string' ||
            typeof range.end !== 'string' ||
            !timePattern.test(range.start) ||
            !timePattern.test(range.end) ||
      ...
      return [
        ...
        invalidBreak
          ...
          : {
              ...
              breaks: (schedule.breaks as Array<Record<string, string>>).map(({ start, end }) => ({
                start,
                end,
              })),
            },
  ...
  ) as PublicBusinessResponse['staff'][number]['schedule'];
}

function publicBusinessDto(
  id: string,
  business: DocumentData,
  now: Date,
): PublicBusinessResponse['business'] {
  const config =
    business.config && typeof business.config === 'object'
      ? (business.config as Record<string, unknown>)
      : {};
  const socialLinks =
    config.socialLinks && typeof config.socialLinks === 'object'
  ...
  const location =
    config.location && typeof config.location === 'object'
      ? (config.location as Record<string, unknown>)
      : {};
  const latitude = location.latitude;
  const longitude = location.longitude;
  const expiresAt =
    business.planExpiresAt &&
    typeof business.planExpiresAt === 'object' &&
    'toDate' in business.planExpiresAt &&
    typeof (business.planExpiresAt as { toDate?: unknown }).toDate === 'function'
      ? (business.planExpiresAt as { toDate: () => Date }).toDate()
      : undefined;
  return {
    id,
    ...
    ...(expiresAt && Number.isFinite(expiresAt.getTime()) && expiresAt >= now
      ? { bookingEnabledUntil: expiresAt.toISOString() }
      : {}),
    config: {
      ...
      ...(Object.keys(socialLinks).length
        ? {
            socialLinks: {
              ...(publicString(socialLinks.instagram)
                ? { instagram: publicString(socialLinks.instagram) }
                : {}),
              ...(publicString(socialLinks.facebook)
                ? { facebook: publicString(socialLinks.facebook) }
                : {}),
              ...(publicString(socialLinks.whatsapp)
                ? { whatsapp: publicString(socialLinks.whatsapp) }
                : {}),
            },
          }
        : {}),
...
function publicServiceDto(
  id: string,
  service: DocumentData,
): PublicBusinessResponse['services'][number] | null {
  ...
  const staffIds = Array.isArray(service.staffIds)
    ? service.staffIds.flatMap((staffId: unknown) =>
        publicString(staffId, 150) ? [publicString(staffId, 150)!] : [],
      )
    : undefined;
  return {
    id,
...
function publicProductDto(
...
): PublicBusinessResponse['products'][number] | null {
  ...
  return {
    ...
    name,
    price: product.price,
    ...(publicString(product.description, 1_000)
      ? { description: publicString(product.description, 1_000) }
      : {}),
...
>(async (request) => {
  ...
  if (!slug || slug.length > 160)
    throw new HttpsError('invalid-argument', 'Invalid public business request.');

  const now = new Date();
  const businesses = await db.collection('barbers').where('slug', '==', slug).limit(2).get();
  if (businesses.size !== 1) throw publicUnavailable();
...
function normalizeBookingPhone(value: string): string | null {
...
function requestFingerprint(input: BookingRequest, normalizedPhone: string): string {
  return hash(
    JSON.stringify({
...
function slotIds(
  ...
  buffer: unknown,
  interval: unknown,
): Array<{ id: string; startTime: string; endTime: string }> | null {
  ...
  for (let value = startMinutes; value < end; value += interval)
    result.push({
      ...
      endTime: format(Math.min(value + interval, 24 * 60)),
    });
  return result;
}

function hasClosure(
  booking: EffectiveBookingSettings,
  date: string,
  start: number,
  end: number,
): boolean {
...
function validCustomerFields(request: BookingRequest, booking: EffectiveBookingSettings): boolean {
  const fields = booking.customerFields;
  const valid = (
    key: 'clientEmail' | 'clientAddress',
    ...
    return state === 'disabled'
      ...
      : value.length <= max &&
          ...
          (state !== 'required' || value.length > 0);
  };
  const extra = fields.email !== 'disabled' || fields.address !== 'disabled';
  return (
    valid('clientEmail', 'email', 254, (value) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) &&
    valid('clientAddress', 'address', 240) &&
    (!extra || request.acceptedBookingPrivacy === true)
  );
}
...
async function canonicalProducts(
  ...
  enabled: boolean,
  products: unknown,
): Promise<Array<{ productId: string; name: string; quantity: number }>> {
  if (products === undefined) return [];
  if (!enabled || !Array.isArray(products) || products.length > 10)
    throw new HttpsError('invalid-argument', 'Invalid product request.');
  const requests = products as ProductRequest[];
  const ids = new Set<string>();
  if (
    !requests.every(
      (product) =>
        ...
        product.productId.length <= 150 &&
        (product.name === undefined || typeof product.name === 'string') &&
        Number.isInteger(product.quantity) &&
        product.quantity >= 1 &&
        product.quantity <= 10 &&
        !ids.has(product.productId) &&
        Boolean(ids.add(product.productId)),
    )
  )
    throw new HttpsError('invalid-argument', 'Invalid product request.');
  const snapshots = await Promise.all(
    requests.map((product) =>
      transaction.get(db.doc(`barbers/${businessId}/products/${product.productId}`)),
    ),
  );
  return snapshots.map((snapshot, index) => {
    ...
    if (
      ...
      data?.active !== true ||
      !name ||
      (requests[index].name !== undefined && requests[index].name.trim() !== name)
    )
      throw new HttpsError('failed-precondition', 'Requested product is no longer available.');
    return { productId: requests[index].productId, name, quantity: requests[index].quantity };
  });
}

export const createPublicBooking = onCall(async (request) => {
  const input = request.data as BookingRequest;
  const normalizedPhone =
    typeof input?.clientPhone === 'string' ? normalizeBookingPhone(input.clientPhone) : null;
  if (
    !input ||
    typeof input.businessId !== 'string' ||
    !validDate(input.bookingDate) ||
    minutes(input.startTime) === null ||
    typeof input.clientName !== 'string' ||
    !input.clientName.trim() ||
    input.clientName.trim().length > 120 ||
    !normalizedPhone ||
  ...
  const idempotencyRef = db.doc(
    `barbers/${input.businessId}/bookingIdempotency/${hash(input.idempotencyKey)}`,
  );
  const phoneDayRef = db.doc(
    `barbers/${input.businessId}/bookingPhoneDays/${hash(`${normalizedPhone}:${input.bookingDate}`)}`,
  );
  ...
  const appointmentId = await db.runTransaction(async (transaction) => {
    ...
    const [businessSnapshot, serviceSnapshot, idempotencySnapshot, phoneDaySnapshot] =
      await Promise.all([
        transaction.get(businessRef),
        transaction.get(serviceRef),
        transaction.get(idempotencyRef),
        transaction.get(phoneDayRef),
      ]);
    if (idempotencySnapshot.exists) {
      const completed = idempotencySnapshot.data();
      if (completed?.fingerprint !== fingerprint || typeof completed.appointmentId !== 'string')
        throw new HttpsError('already-exists', 'A booking request has already been submitted.');
    ...
    if (
      ...
      throw new HttpsError('failed-precondition', 'Booking is unavailable.');
    if (
      !validCustomerFields(input, booking) ||
      (booking.productSelectionEnabled !== true && input.requestedProducts !== undefined)
    )
      throw new HttpsError('invalid-argument', 'Invalid booking request.');
    const bufferMinutes = serviceBufferMinutes(service.bufferMinutes);
    const intervals = slotIds(
      input.startTime,
      service.duration,
      bufferMinutes,
    ...
    const dayOffset =
      (Date.parse(`${input.bookingDate}T00:00:00Z`) - Date.parse(`${today}T00:00:00Z`)) /
      86_400_000;
    if (
      !intervals ||
      !requestedAt ||
      requestedAt.getTime() < Date.now() + booking.minimumNoticeMinutes * 60_000 ||
      dayOffset < 0 ||
      dayOffset > booking.bookingHorizonDays
    )
      throw new HttpsError('failed-precondition', 'Booking is unavailable.');
    ...
    if (phoneDaySnapshot.exists) {
      ...
      const existingAppointment =
        typeof existingAppointmentId === 'string'
          ? await transaction.get(
              db.doc(`barbers/${input.businessId}/appointments/${existingAppointmentId}`),
            )
          : null;
      if (
        existingAppointment?.exists &&
        blockingAppointmentStatuses.has(existingAppointment.data()?.status)
      ) {
        throw new HttpsError(
          'already-exists',
          'A booking already exists for this phone on this date.',
        );
      }
    }
    const products = await canonicalProducts(
      transaction,
      input.businessId,
      booking.productSelectionEnabled === true,
      input.requestedProducts,
    );
    const staffSnapshot = await transaction.get(
      db.collection(`barbers/${input.businessId}/barbers`),
    );
    const candidates = staffSnapshot.docs
      .filter((staff) => {
        const data = staff.data();
        if (
          data.active !== true ||
          (Array.isArray(service.staffIds) && !service.staffIds.includes(staff.id))
        )
          return false;
        ...
        const schedule = data.schedule?.[weekday] || {
          enabled: businessDay?.enabled === true,
        ...
        const overlapsBreak =
          Array.isArray(schedule.breaks) &&
          schedule.breaks.some((item: unknown) => {
            if (!item || typeof item !== 'object') return true;
            const range = item as Record<string, unknown>;
    ...
    if (!candidates.length) throw new HttpsError('failed-precondition', 'Booking is unavailable.');
    for (const staff of candidates) {
      const lockRefs = intervals.map((interval) =>
        db.doc(
          `barbers/${input.businessId}/bookingLocks/${input.bookingDate}/staff/${staff.id}/intervals/${interval.id}`,
        ),
      );
      ...
      transaction.set(appointmentRef, {
        ...
        ...(input.anyProfessional
          ? { assignmentState: 'unassigned', capacityStaffId: staff.id }
          : { barberId: staff.id }),
        ...(input.clientEmail?.trim() ? { clientEmail: input.clientEmail.trim() } : {}),
        ...(input.clientAddress?.trim() ? { clientAddress: input.clientAddress.trim() } : {}),
        ...(booking.customerFields?.email !== 'disabled' ||
        booking.customerFields?.address !== 'disabled'
          ? {
          ...
          : {}),
        ...(products.length ? { requestedProducts: products } : {}),
        ...(input.notes?.trim() ? { notes: input.notes.trim() } : {}),
        ...(request.auth ? { customerUid: request.auth.uid } : {}),
        startTime: input.startTime,
        endTime: intervals.at(-1)!.endTime,
        occupiedIntervalIds: intervals.map((interval) => interval.id),
        primaryLockId: intervals[0].id,
        status: 'pending',
...
function appointmentInput(value: unknown): { businessId: string; appointmentId: string } {
  const input = value as { businessId?: unknown; appointmentId?: unknown };
  if (
    typeof input?.businessId !== 'string' ||
    !input.businessId ||
    ...
    throw new HttpsError('invalid-argument', 'Invalid appointment request.');
...
async function actorForBusiness(
  ...
  const belongsToBusiness =
    Array.isArray(actor?.businessIds) && actor.businessIds.includes(businessId);
  const isStaff =
    actor?.role === 'staff' &&
    staffId === actorId &&
    Array.isArray(actor?.businessIds) &&
...
function canSetAppointmentStatus(appointment: DocumentData | undefined, status: string): boolean {
  if (!appointment) return false;
  if (status === 'confirmed') return appointment.status === 'pending';
  if (
    !['done', 'no_show', 'cancelled'].includes(status) ||
    !['pending', 'confirmed'].includes(appointment.status)
  )
    return false;
  const endedAt = businessLocalDateTime(appointment.bookingDate, appointment.endTime);
  if (endedAt !== null && endedAt.getTime() <= Date.now()) return true;
...
export const cancelCustomerAppointment = onCall(async (request) => {
  ...
  const cancellationNote = parseCancellationNote(
    (request.data as { cancellationNote?: unknown }).cancellationNote,
  );
...
export const updateAppointmentStatus = onCall(async (request) => {
  ...
  if (!['confirmed', 'done', 'no_show', 'cancelled'].includes(status as string))
    throw new HttpsError('invalid-argument', 'Invalid appointment status.');
  const cancellationNote =
    status === 'cancelled'
      ? parseCancellationNote((request.data as { cancellationNote?: unknown }).cancellationNote)
      : '';
  ...
  await db.runTransaction(async (transaction) => {
    ...
    const staffCanUpdate =
      appointment?.barberId === staffId ||
      (appointment?.assignmentState === 'unassigned' && status === 'confirmed');
    if (!canSetAppointmentStatus(appointment, status as string))
      throw new HttpsError('failed-precondition', 'Invalid appointment status transition.');
    if (!appointment)
      throw new HttpsError('failed-precondition', 'Appointment is no longer available.');
    if (isStaff && !staffCanUpdate)
      throw new HttpsError('permission-denied', 'Not authorized to update this appointment.');
    ...
    const intervalIds = Array.isArray(appointment?.occupiedIntervalIds)
      ...
      : [];
    const lockRefs =
      ['done', 'no_show', 'cancelled'].includes(status as string) && lockOwnerId
        ? intervalIds.map((intervalId) =>
            db.doc(
              `barbers/${businessId}/bookingLocks/${appointment.bookingDate}/staff/${lockOwnerId}/intervals/${intervalId}`,
            ),
...
export const claimAppointment = onCall(async (request) => {
  if (!request.auth) throw new HttpsError('unauthenticated', 'Authentication is required.');
  const { businessId, appointmentId } = appointmentInput(request.data);

  await db.runTransaction(async (transaction) => {
    const { staffId, canSelfAssign } = await actorForBusiness(
      transaction,
      request.auth!.uid,
      businessId,
    );
    ...
    const intervalIds = Array.isArray(appointment.occupiedIntervalIds)
      ? appointment.occupiedIntervalIds.filter((id): id is string => typeof id === 'string')
      : [];
    const capacityStaffId =
      typeof appointment.capacityStaffId === 'string' ? appointment.capacityStaffId : '';
    if (!capacityStaffId || !intervalIds.length)
      throw new HttpsError('failed-precondition', 'Appointment is no longer available.');

    const capacityLockRefs = intervalIds.map((intervalId) =>
      db.doc(
        `barbers/${businessId}/bookingLocks/${appointment.bookingDate}/staff/${capacityStaffId}/intervals/${intervalId}`,
      ),
    );
    const staffLockRefs = intervalIds.map((intervalId) =>
      db.doc(
        `barbers/${businessId}/bookingLocks/${appointment.bookingDate}/staff/${staffId}/intervals/${intervalId}`,
      ),
    );
