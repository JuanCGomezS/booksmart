...
type PublicBusinessResponse = {
  ...
  staff: Array<{
    ...
    name: string;
    active: true;
    schedule?: Record<string, { enabled: boolean; start: string; end: string; breaks: Array<{ start: string; end: string }> }>;
  }>;
};
...
function positiveInteger(value: unknown, fallback: number, minimum = 0, maximum = Number.MAX_SAFE_INTEGER): number {
...
function bookingCustomerFields(value: unknown): BookingCustomerFields {
  ...
  const field = (name: keyof BookingCustomerFields): BookingCustomerFieldState =>
    configured[name] === 'required' || configured[name] === 'optional' ? configured[name] : 'disabled';
...
function isBusinessOperational(business: DocumentData | undefined, now = new Date()): business is DocumentData {
  ...
  const toDate = (value: unknown): Date | undefined => value instanceof Date
    ? value
  ...
  if (business.subscriptionStartsAt !== undefined && (!startsAt || !Number.isFinite(startsAt.getTime()) || startsAt > now)) return false;
...
function publicWorkingHours(value: unknown): Record<string, { open: string; close: string; enabled: boolean }> {
  ...
  return Object.fromEntries(Array.from({ length: 7 }, (_, day) => {
    ...
    if (!schedule || typeof schedule !== 'object') return [day, { open: '09:00', close: '18:00', enabled: false }];
...
type PublicClosureRule = { id: string; kind: 'weekly'; weekday: number; startTime?: string; endTime?: string } |
...
function publicClosureRules(booking: EffectiveBookingSettings): PublicClosureRule[] {
  ...
  for (const rule of booking.closureRules) {
    ...
    if (!id || ids.has(id) || (startTime === undefined) !== (endTime === undefined) || (startTime && startTime >= endTime!)) continue;
  ...
  for (const closure of booking.exceptionalClosures) {
    const date = closure && typeof closure === 'object' ? (closure as Record<string, unknown>).date : undefined;
...
function publicStaffSchedule(value: unknown): PublicBusinessResponse['staff'][number]['schedule'] {
  ...
  return Object.fromEntries(Array.from({ length: 7 }, (_, day) => {
    ...
    if (!rawDay || typeof rawDay !== 'object') return [day, { enabled: false, start: '09:00', end: '18:00', breaks: [] }];
    ...
    if (!start || !end || start >= end) return [day, { enabled: false, start: '09:00', end: '18:00', breaks: [] }];
    ...
    return [day, invalidBreak
      ...
      : { enabled: schedule.enabled === true, start, end, breaks: (schedule.breaks as Array<Record<string, string>>).map(({ start, end }) => ({ start, end })) }];
  }).filter(([, schedule]) => schedule !== undefined)) as PublicBusinessResponse['staff'][number]['schedule'];
}
...
function publicBusinessDto(id: string, business: DocumentData, now: Date): PublicBusinessResponse['business'] {
  ...
  const expiresAt = business.planExpiresAt && typeof business.planExpiresAt === 'object' && 'toDate' in business.planExpiresAt && typeof (business.planExpiresAt as { toDate?: unknown }).toDate === 'function'
...
function publicServiceDto(id: string, service: DocumentData): PublicBusinessResponse['services'][number] | null {
  const name = publicString(service.name, 120);
  if (!name || service.active !== true || !Number.isInteger(service.duration) || service.duration <= 0) return null;
  const staffIds = Array.isArray(service.staffIds)
    ? service.staffIds.flatMap((staffId: unknown) => publicString(staffId, 150) ? [publicString(staffId, 150)!] : [])
    : undefined;
...
function publicStaffDto(id: string, staff: DocumentData): PublicBusinessResponse['staff'][number] | null {
...
function publicProductDto(id: string, product: DocumentData): PublicBusinessResponse['products'][number] | null {
  const name = publicString(product.name, 120);
  if (!name || typeof product.price !== 'number' || !Number.isFinite(product.price) || product.price < 0) return null;
  const tags = Array.isArray(product.tags)
    ? product.tags.flatMap((tag: unknown) => publicString(tag, 80) ? [publicString(tag, 80)!] : []).slice(0, 20)
    : [];
  return {
    id,
    name,
...
export const getPublicBusinessBySlug = onCall<PublicBusinessRequest, Promise<PublicBusinessResponse>>(async (request) => {
  const slug = typeof request.data?.slug === 'string' ? request.data.slug.trim() : '';
  if (!slug || slug.length > 160) throw new HttpsError('invalid-argument', 'Invalid public business request.');
...
function slotIds(start: string, duration: unknown, buffer: unknown, interval: unknown): Array<{ id: string; startTime: string; endTime: string }> | null {
  const startMinutes = minutes(start);
  if (startMinutes === null || typeof duration !== 'number' || !Number.isInteger(duration) || duration <= 0 || typeof buffer !== 'number' || !Number.isInteger(buffer) || buffer < 0 || typeof interval !== 'number' || !Number.isInteger(interval) || interval < 5 || interval > 120 || startMinutes % interval !== 0) return null;
  ...
  const result = [];
  for (let value = startMinutes; value < end; value += interval) result.push({ id: format(value).replace(':', ''), startTime: format(value), endTime: format(Math.min(value + interval, 24 * 60)) });
  return result;
}

function hasClosure(booking: EffectiveBookingSettings, date: string, start: number, end: number): boolean {
  const weekday = new Date(`${date}T12:00:00Z`).getUTCDay();
  return publicClosureRules(booking).some((rule) => {
    if ((rule.kind === 'weekly' && rule.weekday !== weekday) || (rule.kind === 'date' && rule.date !== date)) return false;
...
function validCustomerFields(request: BookingRequest, booking: EffectiveBookingSettings): boolean {
  ...
  const valid = (key: 'clientEmail' | 'clientAddress', setting: 'email' | 'address', max: number, check?: (value: string) => boolean) => {
...
async function canonicalProducts(transaction: FirebaseFirestore.Transaction, businessId: string, enabled: boolean, products: unknown): Promise<Array<{ productId: string; name: string; quantity: number }>> {
  if (products === undefined) return [];
  if (!enabled || !Array.isArray(products) || products.length > 10) throw new HttpsError('invalid-argument', 'Invalid product request.');
  const requests = products as ProductRequest[];
  const ids = new Set<string>();
  if (!requests.every((product) => typeof product?.productId === 'string' && product.productId.length > 0 && product.productId.length <= 150 && (product.name === undefined || typeof product.name === 'string') && Number.isInteger(product.quantity) && product.quantity >= 1 && product.quantity <= 10 && !ids.has(product.productId) && Boolean(ids.add(product.productId)))) throw new HttpsError('invalid-argument', 'Invalid product request.');
  ...
  return snapshots.map((snapshot, index) => {
    ...
    if (!snapshot.exists || data?.active !== true || !name || (requests[index].name !== undefined && requests[index].name.trim() !== name)) throw new HttpsError('failed-precondition', 'Requested product is no longer available.');
...
export const createPublicBooking = onCall(async (request) => {
  ...
  const appointmentId = await db.runTransaction(async (transaction) => {
    ...
    if (idempotencySnapshot.exists) {
      ...
      if (completed?.fingerprint !== fingerprint || typeof completed.appointmentId !== 'string') throw new HttpsError('already-exists', 'A booking request has already been submitted.');
      return completed.appointmentId;
    }
    const business = businessSnapshot.data();
    const service = serviceSnapshot.data();
    const booking = effectiveBookingSettings(business?.config?.booking);
    if (!businessSnapshot.exists || !isBusinessOperational(business) || !serviceSnapshot.exists || service?.active !== true) throw new HttpsError('failed-precondition', 'Booking is unavailable.');
    if (!validCustomerFields(input, booking) || (booking.productSelectionEnabled !== true && input.requestedProducts !== undefined)) throw new HttpsError('invalid-argument', 'Invalid booking request.');
    ...
    if (!intervals || !requestedAt || requestedAt.getTime() < Date.now() + booking.minimumNoticeMinutes * 60_000 || dayOffset < 0 || dayOffset > booking.bookingHorizonDays) throw new HttpsError('failed-precondition', 'Booking is unavailable.');
    const start = minutes(input.startTime)!;
    const end = start + Number(service.duration) + bufferMinutes;
    if (hasClosure(booking, input.bookingDate, start, end)) throw new HttpsError('failed-precondition', 'Booking is unavailable.');
    if (phoneDaySnapshot.exists) {
      ...
      if (existingAppointment?.exists && blockingAppointmentStatuses.has(existingAppointment.data()?.status)) {
        throw new HttpsError('already-exists', 'A booking already exists for this phone on this date.');
      }
    }
    ...
    const candidates = staffSnapshot.docs.filter((staff) => {
      ...
      if (data.active !== true || (Array.isArray(service.staffIds) && !service.staffIds.includes(staff.id))) return false;
      ...
      const scheduleStart = minutes(schedule.start); const scheduleEnd = minutes(schedule.end);
      const businessStart = minutes(businessDay?.open); const businessEnd = minutes(businessDay?.close);
      const overlapsBreak = Array.isArray(schedule.breaks) && schedule.breaks.some((item: unknown) => {
        if (!item || typeof item !== 'object') return true;
        const range = item as Record<string, unknown>;
        const breakStart = minutes(range.start); const breakEnd = minutes(range.end);
    ...
    for (const staff of candidates) {
      ...
      transaction.set(appointmentRef, {
        ...
        ...(request.auth ? { customerUid: request.auth.uid } : {}), startTime: input.startTime, endTime: intervals.at(-1)!.endTime, occupiedIntervalIds: intervals.map((interval) => interval.id), primaryLockId: intervals[0].id, status: 'pending', createdAt: FieldValue.serverTimestamp(),
      });
      lockRefs.forEach((reference, index) => transaction.set(reference, { appointmentId: appointmentRef.id, bookingDate: input.bookingDate, staffId: staff.id, intervalId: intervals[index].id, startTime: intervals[index].startTime, endTime: intervals[index].endTime, createdAt: FieldValue.serverTimestamp() }));
      transaction.set(phoneDayRef, { appointmentId: appointmentRef.id, bookingDate: input.bookingDate, createdAt: FieldValue.serverTimestamp() });
      transaction.set(idempotencyRef, { appointmentId: appointmentRef.id, fingerprint, createdAt: FieldValue.serverTimestamp() });
