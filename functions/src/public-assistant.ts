...
function publicWorkingHours(value: unknown) {
  ...
  return Object.fromEntries(
    Array.from({ length: 7 }, (_, day) => {
      ...
      const open =
        typeof schedule.open === 'string' && timePattern.test(schedule.open)
          ? schedule.open
          : '09:00';
      const close =
        typeof schedule.close === 'string' && timePattern.test(schedule.close)
          ? schedule.close
          : '18:00';
...
function visitorKey(request: CallableRequest<unknown>) {
  if (request.auth?.uid) return hash(`uid:${request.auth.uid}`);
  const ip = (request.rawRequest as { ip?: unknown }).ip;
  return hash(`ip:${typeof ip === 'string' ? ip : 'unknown'}`);
}

async function reserveProviderFailureSlot(businessId: string, request: CallableRequest<unknown>) {
  const window = Math.floor(Date.now() / providerFailureWindowMs);
  const failureRef = getFirestore()
  ...
  await getFirestore().runTransaction(async (transaction) => {
    ...
    if (failures + pending >= providerFailureLimit)
      throw new HttpsError('resource-exhausted', 'The AI provider is temporarily unavailable.');
    transaction.set(
      failureRef,
      { pending: pending + 1, updatedAt: FieldValue.serverTimestamp() },
      { merge: true },
    );
  });
  return failureRef;
...
async function settleProviderFailureSlot(failureRef: DocumentReference, failed: boolean) {
  await getFirestore().runTransaction(async (transaction) => {
    const data = (await transaction.get(failureRef)).data();
    const pending = typeof data?.pending === 'number' ? data.pending : 0;
    const failures = typeof data?.failures === 'number' ? data.failures : 0;
...
async function reserveQuestion(businessId: string, request: CallableRequest<unknown>) {
  ...
  await db.runTransaction(async (transaction) => {
    ...
    const visitorTotal =
      typeof visitor.data()?.questions === 'number' ? visitor.data()!.questions : 0;
    if (total >= assistantBusinessDailyLimit || visitorTotal >= assistantDailyLimit)
      throw new HttpsError('resource-exhausted', 'The daily question limit has been reached.');
    transaction.set(
      dailyRef,
      { businessId, day, questions: total + 1, updatedAt: FieldValue.serverTimestamp() },
      { merge: true },
    );
    transaction.set(
      visitorRef,
      { questions: visitorTotal + 1, updatedAt: FieldValue.serverTimestamp() },
      { merge: true },
    );
  });
...
async function releaseQuestionReservation(
  reservation: Awaited<ReturnType<typeof reserveQuestion>>,
) {
  const db = getFirestore();
  await db.runTransaction(async (transaction) => {
    const [daily, visitor] = await Promise.all([
    ...
    if (total > 0)
      transaction.set(
        reservation.dailyRef,
        { questions: total - 1, updatedAt: FieldValue.serverTimestamp() },
        { merge: true },
      );
    if (visitorTotal > 0)
      transaction.set(
        reservation.visitorRef,
        { questions: visitorTotal - 1, updatedAt: FieldValue.serverTimestamp() },
        { merge: true },
      );
  });
...
async function complete(
  ...
  maxTokens = 1_024,
  maximumResponseLength = 2_000,
) {
  const apiKey = openRouterApiKey.value();
  if (!apiKey) throw new HttpsError('failed-precondition', 'The AI integration is not configured.');
  ...
  throw new HttpsError('unavailable', 'The AI provider is unavailable.');
}

async function requireBusinessManager(uid: string, businessId: string) {
  const actor = (await getFirestore().doc(`users/${uid}`).get()).data();
...
async function sourceForSlug(slug: string, fallbackContext?: string): Promise<AssistantSource> {
  const db = getFirestore();
  const businesses = await db.collection('barbers').where('slug', '==', slug).limit(2).get();
  if (businesses.size !== 1) throw new HttpsError('not-found', 'Business is unavailable.');
  const businessSnapshot = businesses.docs[0];
  const business = businessSnapshot.data();
  if (!isOperational(business))
    throw new HttpsError('permission-denied', 'Business is unavailable.');
  const config =
    business.config && typeof business.config === 'object'
      ? (business.config as Record<string, unknown>)
      : {};
  const context =
    text(config.publicAssistantProfile, 6_000) ||
    text(config.publicAssistantContext, 6_000) ||
    fallbackContext ||
    '';
  if (!context) throw new HttpsError('not-found', 'Assistant is unavailable.');
...
async function publishedProfileForSlug(slug: string) {
  const cached = profileCache.get(slug);
  if (cached && cached.expiresAt > Date.now()) return cached;
  const businesses = await getFirestore()
    .collection('barbers')
    .where('slug', '==', slug)
    .limit(2)
    .get();
  ...
  if (!isOperational(business.data()))
    throw new HttpsError('permission-denied', 'Business is unavailable.');
  const config = business.data().config as Record<string, unknown> | undefined;
  const context =
    text(config?.publicAssistantProfile, 6_000) || text(config?.publicAssistantContext, 6_000);
  if (!context) throw new HttpsError('not-found', 'Assistant is unavailable.');
...
export const improvePublicAssistantContext = onCall(
  { secrets: [openRouterApiKey] },
  async (request): Promise<{ context: string }> => {
    if (!request.auth) throw new HttpsError('unauthenticated', 'Authentication is required.');
    const input = request.data as { businessId?: unknown; context?: unknown };
    const businessId = text(input?.businessId, 150);
    const context = text(input?.context, 6_000);
    if (!businessId || !context)
      throw new HttpsError('invalid-argument', 'Invalid assistant context.');
    await requireBusinessManager(request.auth.uid, businessId);
    const business = await getFirestore().doc(`barbers/${businessId}`).get();
    const slug = text(business.data()?.slug, 160);
    if (!business.exists || !slug) throw new HttpsError('not-found', 'Business is unavailable.');
        ...
        [
          {
            role: 'system',
            content:
              'Eres un editor para la página pública de un negocio de servicios. Crea un perfil público compacto en español claro y ordenado, de máximo 5.000 caracteres. Conserva únicamente los hechos proporcionados; no inventes precios, horarios, servicios, políticas ni promesas. Devuelve solo el texto final, sin títulos, notas ni markdown.',
          },
          {
            role: 'user',
...
export const askPublicBusinessAssistant = onCall(
  { enforceAppCheck: true, secrets: [openRouterApiKey] },
  async (request): Promise<{ answer: string }> => {
    const input = request.data as { slug?: unknown; question?: unknown };
    const slug = text(input?.slug, 160);
    const question = text(input?.question, 500);
    if (!slug || !question) throw new HttpsError('invalid-argument', 'Invalid assistant question.');
    ...
    let reservation: Awaited<ReturnType<typeof reserveQuestion>> | undefined;
    try {
