...
exports.claimStoreadminCapacityAppointment =
  exports.createPublicBooking =
  exports.getPublicBusinessBySlug =
    void 0;
...
function publicThemeDto(value) {
  ...
  if (
    ...
    keys.every((key) => typeof palette[key] === 'string' && hexColorPattern.test(palette[key]))
  ) {
...
function publicBusinessDto(id, business, now) {
  ...
  const socialLinks =
    config.socialLinks && typeof config.socialLinks === 'object' ? config.socialLinks : {};
  ...
  const expiresAt =
    ...
    typeof business.planExpiresAt.toDate === 'function'
...
function publicServiceDto(id, service) {
  ...
  const staffIds = Array.isArray(service.staffIds)
    ? service.staffIds.flatMap((staffId) =>
  ...
  return snapshots.map((snapshot, index) => {
    ...
    if (
      ...
      throw new https_1.HttpsError(
        'failed-precondition',
        'Requested product is no longer available.',
      );
  ...
  const appointmentId = await db.runTransaction(async (transaction) => {
    ...
    if (idempotencySnapshot.exists) {
      ...
      if (completed?.fingerprint !== fingerprint || typeof completed.appointmentId !== 'string')
        throw new https_1.HttpsError(
          'already-exists',
          'A booking request has already been submitted.',
        );
...
exports.claimStoreadminCapacityAppointment = (0, https_1.onCall)(async (request) => {
  ...
  await db.runTransaction(async (transaction) => {
    ...
    if (
      ...
      throw new https_1.HttpsError(
        'permission-denied',
        'Not authorized to claim this appointment.',
      );
    }
