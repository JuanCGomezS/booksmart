...
function bookingCustomerFields(value) {
    ...
    const field = (name) => configured[name] === 'required' || configured[name] === 'optional'
        ? configured[name]
        : 'disabled';
...
function isBusinessOperational(business, now = new Date()) {
    ...
    if (business.subscriptionStartsAt !== undefined &&
...
function publicThemeDto(value) {
    ...
    if (typeof theme.id === 'string' && publicPresetThemeIds.has(theme.id))
        return { id: theme.id };
    const palette = theme.palette && typeof theme.palette === 'object' && !Array.isArray(theme.palette)
        ? theme.palette
        : {};
    ...
    if (theme.id === 'custom' &&
        ...
        return {
            id: 'custom',
            palette: Object.fromEntries(keys.map((key) => [key, palette[key]])),
        };
    }
...
function publicClosureRules(booking) {
    ...
    for (const rule of booking.closureRules) {
        ...
        const startTime = typeof value.startTime === 'string' && timePattern.test(value.startTime)
            ? value.startTime
            : undefined;
        const endTime = typeof value.endTime === 'string' && timePattern.test(value.endTime)
            ? value.endTime
            : undefined;
        if (!id ||
            ids.has(id) ||
    ...
    for (const closure of booking.exceptionalClosures) {
        const date = closure && typeof closure === 'object'
            ? closure.date
            : undefined;
...
function publicStaffSchedule(value) {
    ...
    return Object.fromEntries(Array.from({ length: 7 }, (_, day) => {
        ...
        const start = typeof schedule.start === 'string' && timePattern.test(schedule.start)
            ? schedule.start
            : undefined;
        const end = typeof schedule.end === 'string' && timePattern.test(schedule.end)
            ? schedule.end
            : undefined;
        if (!start || !end || start >= end)
            return [day, { enabled: false, start: '09:00', end: '18:00', breaks: [] }];
        const invalidBreak = !Array.isArray(schedule.breaks) ||
        ...
        return [
            ...
            invalidBreak
                ...
                : {
                    ...
                    breaks: schedule.breaks.map(({ start, end }) => ({
                        start,
                        end,
                    })),
                },
...
function publicBusinessDto(id, business, now) {
    const config = business.config && typeof business.config === 'object'
        ? business.config
        : {};
    const socialLinks = config.socialLinks && typeof config.socialLinks === 'object'
        ? config.socialLinks
        : {};
    const booking = effectiveBookingSettings(config.booking);
    const location = config.location && typeof config.location === 'object'
        ? config.location
        : {};
...
function hasClosure(booking, date, start, end) {
    ...
    return publicClosureRules(booking).some((rule) => {
        if ((rule.kind === 'weekly' && rule.weekday !== weekday) ||
...
async function canonicalProducts(transaction, businessId, enabled, products) {
    ...
    const ids = new Set();
    if (!requests.every((product) => typeof product?.productId === 'string' &&
...
exports.createPublicBooking = (0, https_1.onCall)(async (request) => {
    ...
    const appointmentId = await db.runTransaction(async (transaction) => {
        ...
        const [businessSnapshot, serviceSnapshot, idempotencySnapshot, phoneDaySnapshot] = await Promise.all([
        ...
        if (!validCustomerFields(input, booking) ||
        ...
        const dayOffset = (Date.parse(`${input.bookingDate}T00:00:00Z`) - Date.parse(`${today}T00:00:00Z`)) /
            86_400_000;
        if (!intervals ||
            !requestedAt ||
        ...
        const candidates = staffSnapshot.docs
            ...
            const overlapsBreak = Array.isArray(schedule.breaks) &&
                schedule.breaks.some((item) => {
                    if (!item || typeof item !== 'object')
                        return true;
                    const range = item;
            ...
            return (businessDay?.enabled === true &&
