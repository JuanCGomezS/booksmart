...
import BarberManagementModal from './BarberManagementModal';
...
export default function BarbersList({ barbers, onRefresh }: BarbersListProps) {
  const [managedBarber, setManagedBarber] = useState<Barber | null>(null);

  useEffect(() => {
    if (managedBarber) setManagedBarber(barbers.find((barber) => barber.id === managedBarber.id) || null);
  }, [barbers]);
