...
export default function BookingConfiguration({ business }: { business: BookingBusiness }) {
  ...
  return <section className="space-y-8">
    ...
    <div><h4 className="font-semibold text-main">Cierres excepcionales</h4><div className="mt-2 flex flex-wrap gap-2"><input type="date" className="field-input" value={closureDate} onChange={(event) => setClosureDate(event.target.value)} /><input className="field-input" value={closureReason} placeholder="Motivo opcional" onChange={(event) => setClosureReason(event.target.value)} /><button type="button" className="btn-outline rounded-lg px-3 py-2 text-sm" onClick={() => { if (!closureDate || settings.exceptionalClosures.some((closure) => closure.date === closureDate)) return setMessage('Elegí una fecha de cierre que no esté repetida.'); setSettings({ ...settings, exceptionalClosures: [...settings.exceptionalClosures, { date: closureDate, reason: closureReason.trim() || undefined }] }); setClosureDate(''); setClosureReason(''); }}>Agregar cierre</button></div><div className="mt-2 space-y-1 text-sm">{settings.exceptionalClosures.map((closure) => <p key={closure.date}>{closure.date}{closure.reason ? `: ${closure.reason}` : ''} <button type="button" className="underline" onClick={() => setSettings({ ...settings, exceptionalClosures: settings.exceptionalClosures.filter((item) => item.date !== closure.date) })}>Quitar</button></p>)}</div></div>
