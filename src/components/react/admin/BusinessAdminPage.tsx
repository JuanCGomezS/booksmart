...
import { getBarberStatus, updateBarberPlanSettings } from '../../../lib/barbers';
import { saveBusinessDetails, validateBusinessBrandingImage } from '../../../lib/business-branding';
import type { Barber, BillingCycle, Plan, PublicBusiness, SubscriptionStatus, UserRole } from '../../../lib/types';
...
type Tab = 'agenda' | 'perfil' | 'horario' | 'negocio' | 'contenido' | 'reservas' | 'suscripcion';
...
type BusinessFormState = {
  ...
  instagram: string; facebook: string; whatsapp: string; publicThemeId: ReturnType<typeof resolvePublicThemeId>;
};
...
export default function BusinessAdminPage({ business, role, staffId, userId, profileName, onBack, onRefresh }: { business: Business; role: UserRole; staffId?: string; userId: string; profileName?: string; onBack: () => void; onRefresh: () => void | Promise<void> }) {
  ...
  const tabs: Array<[Tab, string]> = isStaff ? [['agenda', 'Agenda'], ['perfil', 'Mi perfil profesional'], ['horario', 'Mi horario']] : [['agenda', 'Agenda'], ...(canManageOwnSchedule ? [['perfil', 'Mi perfil profesional'], ['horario', 'Mi horario']] as Array<[Tab, string]> : []), ['negocio', 'Negocio'], ['contenido', 'Contenido'], ['reservas', 'Agendamiento'], ...(global ? [['suscripcion', 'Suscripción']] as Array<[Tab, string]> : [])];
...
function LegacyBusinessForm({ form, setForm, saving, logoUrl, coverUrl, onSave }: { form: BusinessFormState; setForm: React.Dispatch<React.SetStateAction<BusinessFormState>>; saving: boolean; logoUrl?: string; coverUrl?: string; onSave: (files: Record<BrandingSlot, File | null>, onProgress: (progress: Partial<Record<BrandingSlot, number>>) => void, onTask: (task: UploadTask | null) => void) => void }) {
  ...
  const save = () => { setProgress({}); onSave(files, setProgress, (task) => { uploadTask.current = task; }); };
...
function BusinessForm(props: { form: BusinessFormState; setForm: React.Dispatch<React.SetStateAction<BusinessFormState>>; saving: boolean; logoUrl?: string; coverUrl?: string; onSave: (files: Record<BrandingSlot, File | null>, onProgress: (progress: Partial<Record<BrandingSlot, number>>) => void, onTask: (task: UploadTask | null) => void) => void }) {
  return <><LegacyBusinessForm {...props} /></>;
}
