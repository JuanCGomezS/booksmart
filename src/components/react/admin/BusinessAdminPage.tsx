...
type BusinessFormState = {
  ...
  address: string;
  location?: Barber['config']['location'];
  phone: string;
  instagram: string;
  facebook: string;
  whatsapp: string;
...
const PLAN_OPTIONS: FancySelectOption<string>[] = Object.values(DATA.PLAN).map((value) => ({
  value,
  label: PLAN_LABEL[value],
}));
const BILLING_OPTIONS: FancySelectOption<string>[] = Object.values(DATA.BILLING_CYCLE).map(
  (value) => ({ value, label: BILLING_CYCLE_LABEL[value] }),
);
...
export default function BusinessAdminPage({
  ...
  const [form, setForm] = useState<BusinessFormState>(() => ({
    ...
    location: business.config.location,
    phone: business.config.phone || '',
    instagram: business.config.socialLinks?.instagram || '',
    facebook: business.config.socialLinks?.facebook || '',
    whatsapp: business.config.socialLinks?.whatsapp || '',
    publicThemeId: resolvePublicThemeId(business.config.theme?.id),
    customThemePalette:
      validateCustomThemePalette(business.config.theme?.palette).palette ||
      DEFAULT_CUSTOM_THEME_PALETTE,
  }));
  const full = business as Barber;
  const [plan, setPlan] = useState<Plan>(full.plan || DATA.PLAN.STANDARD);
  const [billingCycle, setBillingCycle] = useState<BillingCycle>(
    full.billingCycle || DATA.BILLING_CYCLE.MONTH_1,
  );
  const initialSubscriptionStatus = getInitialSubscriptionStatus(full);
  const [subscriptionStatus, setSubscriptionStatus] =
    useState<SubscriptionStatus>(initialSubscriptionStatus);
  const [subscriptionStartsAt, setSubscriptionStartsAt] = useState(() =>
    toDateInput(getInitialSubscriptionStart(full, billingCycle)),
  );
  ...
  const subscriptionEndDate = subscriptionStartDate
    ? getSubscriptionEndDate(subscriptionStartDate, billingCycle)
    : null;
  ...
  useEffect(() => {
    if (!hasOwnProfessionalProfile && tab === 'horario') setTab('agenda');
  }, [hasOwnProfessionalProfile, tab]);
  useEffect(() => {
    if (isStaff) return;
    let cancelled = false;
  ...
  const run = async (action: () => Promise<boolean>) => {
    ...
    try {
      ...
      notifyError(cause instanceof Error ? cause.message : 'No se pudo guardar el cambio.');
    } finally {
      setSaving(false);
    }
  };
  const saveBusiness = (
    files: Record<BrandingSlot, File | null>,
    onProgress: (progress: Partial<Record<BrandingSlot, number>>) => void,
    onTask: (task: UploadTask | null) => void,
  ) =>
    void run(async () => {
      await saveBusinessDetails(business.id, form, files, onProgress, onTask);
      return true;
    });
  const saveSubscription = () => {
    ...
    void run(() =>
      updateBarberPlanSettings(business.id, plan, billingCycle, {
      ...
      <div className="mx-auto max-w-6xl px-4 py-8">
        <header className="mb-7 flex flex-wrap items-start justify-between gap-4">
          <div>
            <button
              type="button"
              className="mb-3 text-sm font-semibold text-subtle hover:text-main"
              onClick={onBack}
            >
              ← Cambiar negocio
            </button>
            ...
            <h1 className="text-3xl font-bold text-main">{business.name}</h1>
            <p className="mt-1 text-sm text-subtle">
              {isStaff
                ? 'Personal · agenda y horario personal'
                : global
                  ? 'Superadministración · operación y controles globales'
                  : 'Administración · operación del negocio'}
            </p>
          </div>
          <a
          ...
          <div className="flex gap-1 overflow-x-auto">
            {tabs.map(([key, label]) => (
              <button
                ...
                onClick={() => {
                  setTab(key);
                  if (key === 'contenido') setContentActivated(true);
                }}
                className={`shrink-0 border-b-2 px-4 py-3 text-sm font-semibold ${tab === key ? 'border-(--secondary) text-(--secondary)' : 'border-transparent text-subtle'}`}
              >
                {label}
              </button>
            ))}
        ...
        {tab === 'suscripcion' && global && (
          <section
            className="surface-card max-w-2xl rounded-2xl p-6"
            aria-labelledby="subscription-title"
          >
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <h2 id="subscription-title" className="text-xl font-bold text-main">
                  Suscripción
                </h2>
                <p className="mt-1 max-w-prose text-sm text-subtle">
                  Defina el estado, el plan y el periodo de activación de este negocio.
                </p>
              </div>
              <span
                className={`status-badge inline-flex rounded-full px-3 py-1 text-sm font-semibold ${subscriptionStatus === 'active' ? 'status-active' : subscriptionStatus === 'trial' ? 'status-trial' : 'status-inactive'}`}
              >
                Estado: {SUBSCRIPTION_STATUS_LABEL[subscriptionStatus]}
              </span>
            </div>
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              <label className="field-label">
                ...
                <FancySelect
                  ...
                  onChange={(value) => {
                    ...
                    setDisableConfirmationPending(
                      nextStatus === 'disabled' && initialSubscriptionStatus !== 'disabled',
                    );
                  }}
                />
              </label>
              ...
              <label className="field-label">
                ...
                <input
                  ...
                  aria-describedby="subscription-range"
                />
              </label>
            </div>
            <div
              id="subscription-range"
              className="surface-soft mt-5 grid gap-1 rounded-lg p-4 sm:grid-cols-[minmax(9rem,auto)_1fr] sm:items-baseline"
            >
              <p className="text-sm font-semibold text-main">Rango calculado</p>
              <p className="text-sm text-subtle" aria-live="polite">
                {subscriptionStartDate && subscriptionEndDate
                  ? `${subscriptionStartDate.toLocaleDateString('es-CO')} — ${subscriptionEndDate.toLocaleDateString('es-CO')}`
                  : 'Ingrese una fecha de inicio válida.'}
              </p>
            </div>
            {disableConfirmationPending && (
              <div
                className="mt-5 border border-[var(--danger)] bg-[color-mix(in_srgb,var(--danger)_10%,var(--surface))] p-4"
                role="alert"
              >
                <h3 className="font-semibold text-main">Confirmar desactivación</h3>
                <p className="mt-1 text-sm text-subtle">
                  La página pública y sus flujos disponibles dejarán de estar activos. El negocio y
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                  ...
                  <button
                    type="button"
                    className="danger-action rounded-lg px-4 py-2 disabled:opacity-50"
                    disabled={saving}
            ...
            <button
              type="button"
              disabled={saving || disableConfirmationPending}
              className="btn-primary mt-6 rounded-lg px-4 py-2 disabled:opacity-50"
              onClick={saveSubscription}
            >
...
function LegacyBusinessForm({
  ...
  onThemeSelect: (themeId: PublicThemeId) => void;
}) {
  const [files, setFiles] = useState<Record<BrandingSlot, File | null>>({
    logo: null,
    cover: null,
  });
  ...
  const chooseImage = (slot: BrandingSlot, file: File | null) => {
    if (!file) return;
    const error = validateBusinessBrandingImage(file);
    if (error) {
      setImageError(error);
      return;
    }
  ...
  const removeImage = (slot: BrandingSlot) => {
  ...
  const save = () => {
    if (form.publicThemeId === 'custom') {
      const result = validateCustomThemePalette(form.customThemePalette);
      if (!result.palette) {
        notifyError(result.error || 'Revise los colores del tema personalizado.');
        return;
      }
    }
    setProgress({});
    onSave(files, setProgress, (task) => {
  ...
  const field = (
    ...
    key: keyof Pick<
      BusinessFormState,
      'name' | 'address' | 'phone' | 'instagram' | 'facebook' | 'whatsapp'
    >,
    hint?: string,
  ) => {
    return (
      <div className={key === 'address' ? 'md:col-span-2' : undefined}>
        <label className="field-label">
          {label}
          <input
            className="field-input mt-1"
          ...
          />
          {hint && <span className="field-hint mt-1 block text-xs">{hint}</span>}
        </label>
        {key === 'address' && (
  ...
  return (
    <section className="surface-card max-w-4xl rounded-2xl p-6">
      <h2 className="text-xl font-bold text-main">Datos del negocio</h2>
      <p className="mt-1 text-sm text-subtle">
        Personalice la página pública autónoma del negocio. La ubicación actual se muestra con el
        punto seleccionado en el mapa; la dirección es una referencia adicional.
      </p>
      <div className="mt-6 grid gap-3 md:grid-cols-2">
        {field('Nombre', 'name')}
        {field('Dirección', 'address', 'Se muestra como referencia.')}
      ...
      <fieldset className="mt-8 border-t border-(--border) pt-6">
        <legend className="text-lg font-bold text-main">Tema de la página pública</legend>
        <p className="mt-1 text-sm text-subtle">Elija un tema exclusivo para este negocio.</p>
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {PUBLIC_THEMES.map((theme) => (
            <button
              key={theme.id}
              type="button"
              className={`public-theme-choice ${form.publicThemeId === theme.id ? 'is-selected' : ''}`}
              aria-pressed={form.publicThemeId === theme.id}
              onClick={() => {
                onThemeSelect(theme.id);
                setForm({ ...form, publicThemeId: theme.id });
              }}
            >
              <span className="public-theme-swatches">
      ...
      <fieldset className="mt-8 border-t border-(--border) pt-6">
        <legend className="text-lg font-bold text-main">Logo y portada</legend>
        <p className="mt-1 text-sm text-subtle">JPEG, PNG o WebP; máximo 5 MiB.</p>
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          {(['logo', 'cover'] as BrandingSlot[]).map((slot) => {
            const source = previews[slot] || (slot === 'logo' ? logoUrl : coverUrl);
            return (
              <div key={slot} className="business-brand-image-picker">
                <p className="font-semibold text-main">{slot === 'logo' ? 'Logo' : 'Portada'}</p>
                {source ? (
                  <img
                    src={source}
                    alt={`Vista previa de ${slot === 'logo' ? 'logo' : 'portada'}`}
                    className={`mt-3 ${slot === 'logo' ? 'business-brand-logo-preview' : 'business-brand-cover-preview'}`}
                  />
                ) : (
                  <div
                    className={`mt-3 business-brand-image-empty ${slot === 'logo' ? 'business-brand-logo-preview' : 'business-brand-cover-preview'}`}
                  >
                    Sin imagen
                  </div>
                )}
                <label className="btn-outline mt-3 inline-flex cursor-pointer rounded-lg px-3 py-2 text-sm font-semibold">
                  Seleccionar imagen
                  <input
                    className="sr-only"
                    type="file"
                    accept="image/jpeg,image/png,image/webp"
                    onChange={(event) => chooseImage(slot, event.target.files?.[0] || null)}
                  />
                </label>
                {previews[slot] && (
                  <button
                    type="button"
                    className="ml-2 text-sm font-semibold text-subtle underline"
                ...
                {progress[slot] !== undefined && (
                  <p className="mt-2 text-sm text-subtle">Subiendo {progress[slot]}%</p>
                )}
              </div>
            );
          })}
        ...
        {saving && uploadTask.current && (
          <button
            type="button"
            className="btn-outline mt-4 rounded-lg px-3 py-2 text-sm"
            onClick={() => uploadTask.current?.cancel()}
          >
            Cancelar carga
          </button>
        )}
...
function BusinessForm(props: {
  ...
  const [presetBeforeCustom, setPresetBeforeCustom] = useState<PresetPublicThemeId>(
    props.form.publicThemeId === 'custom' ? DEFAULT_PUBLIC_THEME_ID : props.form.publicThemeId,
  );
  ...
  return (
    <>
      <CustomThemePaletteDialog
        ...
        open={open}
        setOpen={setOpen}
        presetBeforeCustom={presetBeforeCustom}
      />
      <LegacyBusinessForm {...props} onThemeSelect={selectTheme} />
    </>
  );
}
...
function CustomThemePaletteDialog({
  ...
  setOpen,
  presetBeforeCustom,
}: Pick<Parameters<typeof BusinessForm>[0], 'form' | 'setForm' | 'saving'> & {
  open: boolean;
  setOpen: React.Dispatch<React.SetStateAction<boolean>>;
  presetBeforeCustom: PresetPublicThemeId;
}) {
  ...
  const update = (key: keyof CustomThemePalette, value: string) =>
    setForm({ ...form, customThemePalette: { ...form.customThemePalette, [key]: value } });
    ...
    >
      <div className="surface-card custom-theme-dialog-content">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 id="custom-theme-title" className="text-xl font-bold text-main">
              Tema personalizado
            </h2>
            <p className="mt-1 text-sm text-subtle">
              Defina los cuatro colores. Guarde el formulario principal para aplicar los cambios.
            </p>
          </div>
          <button
            type="button"
            ...
            <label key={key} className="theme-color-field">
              ...
              <span className="theme-color-inputs">
                <input
                  type="color"
                  value={
                    /^#[0-9a-f]{6}$/i.test(form.customThemePalette[key])
                      ? form.customThemePalette[key]
                      : '#000000'
                  }
                  disabled={saving}
                ...
                <input
                  ...
                  value={form.customThemePalette[key]}
                  disabled={saving}
                  onChange={(event) => update(key, event.target.value)}
                  aria-describedby={`custom-theme-${key}-hint custom-theme-validation`}
                />
              </span>
              <small id={`custom-theme-${key}-hint`}>{hint}</small>
            </label>
          ))}
        </div>
        <p
          id="custom-theme-validation"
          className={`mt-4 text-sm font-semibold ${validation.error ? 'text-danger' : 'text-success'}`}
          role={validation.error ? 'alert' : 'status'}
          aria-live={validation.error ? 'assertive' : 'polite'}
        >
          {validation.error ||
            'Paleta válida. Puede cerrar el editor y guardar el formulario principal.'}
        </p>
        <div className="mt-5 flex flex-wrap justify-end gap-3">
          <button
            type="button"
            className="btn-outline px-4 py-2 text-sm font-semibold"
            onClick={cancelCustomTheme}
            disabled={saving}
          >
            Cancelar
          </button>
