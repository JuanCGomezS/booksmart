...
export default function ContentManagement({ barberId, onChange }: { barberId: string; onChange: () => void }) {
  ...
  return <section className="business-admin-content-management grid gap-6 md:grid-cols-[11rem_minmax(0,1fr)]">
    <nav aria-label="Secciones de contenido" className="business-admin-content-tabs flex gap-1 overflow-x-auto md:flex-col md:overflow-visible">
       {tabs.map(([key, label]) => <button key={key} type="button" role="tab" aria-selected={tab === key} onClick={() => setTab(key)} className={`business-admin-content-tab shrink-0 rounded-lg border-l-2 px-4 py-3 text-left text-sm font-semibold ${tab === key ? 'border-(--secondary) bg-(--surface) text-main' : 'border-transparent text-subtle hover:bg-(--surface)'}`}>{label}</button>)}
    </nav>
