...
import { BOOKING_NOTE_MAX_LENGTH, createClientBooking, normalizeBookingNote, type PublicBookingConfiguration } from '../../../lib/booking-transaction';
import { loadPublicBookingConfiguration, loadPublicDateLocks } from '../../../lib/public-booking';
import type { AppointmentProductRequest, PublicBookingProduct, PublicBookingService, PublicBookingStaff, PublicBusiness } from '../../../lib/types';

type BookingSlot = { time: string; staffId?: string; available: boolean; unavailableReason?: string };
...
function BookingProductThumbnail({ product }: { product: PublicBookingProduct }) {
  ...
  if (product.imageUrl && !failed) {
    return <img className="booking-product-thumbnail" src={product.imageUrl} alt="" loading="lazy" onError={() => setFailed(true)} />;
  }
  return <div className="booking-product-thumbnail booking-product-thumbnail-placeholder" aria-hidden="true">{product.name.slice(0, 1).toLocaleUpperCase('es-CO')}</div>;
}

export default function PublicBookingWidget({ business, products: publicProducts, services: publicServices, staff: publicStaff, whatsappUrl }: { business: PublicBusiness; products: PublicBookingProduct[]; services: PublicBookingService[]; staff: PublicBookingStaff[]; whatsappUrl: string | null }) {
  ...
  const [requestedProductQuantities, setRequestedProductQuantities] = useState<Record<string, number>>({});
  ...
  const getCalendarDateUnavailableReason = (date: string): string | null => {
    ...
    if (isBookingDateFullyClosed(settings || { closureRules: [], exceptionalClosures: [] }, date)) return 'El negocio no recibe solicitudes ese día.';
  ...
  const loadConfiguration = async () => {
    ...
    setLoadError('');
    try {
      const configuration = loadPublicBookingConfiguration(business, publicProducts, publicServices, publicStaff);
      ...
      console.error(error);
      setLoadError('No pudimos cargar la configuración actual de agendamiento. No mostraremos horarios hasta que puedas reintentar.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const slots = useMemo((): BookingSlot[] => {
    ...
    if (!businessDay?.enabled || start === null || end === null || !Number.isInteger(interval) || interval <= 0) return [];
    ...
    for (let minute = start; minute < end; minute += interval) {
      ...
      const evaluations = time && required ? availabilityStaff.map((member) => {
        ...
        if (availability.available === false) return { member, available: false, reason: AVAILABILITY_REASON_LABELS[availability.reason] };
        if (!occupied || hasBookingLockConflict(required.map((item) => item.id), occupied)) return { member, available: false, reason: 'Ocupado.' };
        return { member, available: true };
      }) : [];
      const availableStaff = evaluations.find((evaluation) => evaluation.available);
      if (time) candidates.push({
        time,
        available: Boolean(availableStaff),
        ...(availableStaff ? { staffId: availableStaff.member.id } : { unavailableReason: evaluations[0]?.reason || 'Horario no disponible.' }),
      });
    }
    return candidates;
  ...
  const slotsByPeriod = useMemo(() => slots.reduce<Record<string, BookingSlot[]>>((groups, item) => {
  ...
  const requestedProducts = useMemo((): AppointmentProductRequest[] => selectableProducts.flatMap((product) => {
  ...
  const requestedProductSubtotal = useMemo(() => requestedProducts.reduce((total, item) =>
    total + (selectableProducts.find((product) => product.id === item.productId)?.price || 0) * item.quantity,
  0), [requestedProducts, selectableProducts]);
  ...
  const addressRequested = customerFields?.address !== undefined && customerFields.address !== 'disabled';
  const requiresPrivacyAcceptance = customerFields ? bookingRequestsAdditionalCustomerData(customerFields) : false;
  ...
  const emailValid = !emailValue || (emailValue.length <= 254 && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailValue));
  ...
  const canConfirm = Boolean(slot && nameValid && phoneValid && phoneLengthValid && emailValid && addressValid &&
    bookingNoteValid &&
  ...
  const validationIssues = [
    ...
    (!phoneValid || !phoneLengthValid) && 'ingresa un celular colombiano válido de máximo 40 caracteres',
    emailRequested && (!emailValid || (customerFields?.email === 'required' && !emailValue)) && 'ingresa un correo electrónico válido',
    addressRequested && (!addressValid || (customerFields?.address === 'required' && !addressValue)) && 'ingresa una dirección de máximo 240 caracteres',
    !bookingNoteValid && `reduce la nota a ${BOOKING_NOTE_MAX_LENGTH} caracteres o menos`,
    requiresPrivacyAcceptance && !consent && 'acepta los documentos de privacidad para compartir estos datos',
  ].filter(Boolean) as string[];
  const showValidationToast = () => {
    const requirements = validationIssues.length === 1
      ? validationIssues[0]
  ...
  const submit = async () => {
    ...
    const result = await createClientBooking(
      { businessId: business.id, serviceId: selectedService.id, bookingDate, startTime: slot.time, clientName, clientPhone: clientPhone.replace(/[\s()-]/g, ''), idempotencyKey: idempotencyKeyRef.current, ...(emailRequested ? { clientEmail } : {}), ...(addressRequested ? { clientAddress } : {}), ...(normalizedBookingNote ? { notes: normalizedBookingNote } : {}), ...(requiresPrivacyAcceptance ? { acceptedBookingPrivacy: consent } : {}), ...(productSelectionEnabled && requestedProducts.length ? { requestedProducts } : {}), anyProfessional: !staffId, ...(staffId ? { staffId: slot.staffId } : {}) },
      {
        business,
        service: selectedService,
        staff: staffMember,
  ...
  if (!isPublicBookingAvailable(business)) {
    return <div className="booking-sheet space-y-3" role="status">
      <p className="press-label text-subtle">Agendamiento no disponible</p>
      <h2 className="text-xl font-bold text-main">Este negocio no recibe solicitudes en línea.</h2>
    </div>;
  }
  ...
  if (confirmed && selectedService && bookingDate && slot) {
    return <div className="booking-sheet space-y-4" role="status" aria-live="polite">
      ...
      <dl className="surface-soft grid gap-3 rounded p-4 text-sm sm:grid-cols-2">
        ...
        <div><dt className="text-subtle">Profesional</dt><dd className="font-semibold text-main">{staffId ? confirmedStaff?.name : 'El negocio asignará a tu profesional.'}</dd></div>
  ...
  return <div className="booking-sheet space-y-7" aria-busy={loading || loadingSlots || submitting}>
    {validationToast && <section className="floating-notifications" aria-label="Requisitos para enviar la solicitud">
      <div ref={validationToastRef} tabIndex={-1} className="floating-notification floating-notification-error" role="alert" aria-atomic="true">
        <span className="floating-notification-icon" aria-hidden="true">!</span>
        ...
        <button type="button" className="floating-notification-dismiss" onClick={() => setValidationToast('')} aria-label="Cerrar requisitos">
    ...
    <div>
      ...
      <h2 ref={bookingHeadingRef} tabIndex={-1} className="mt-2 text-2xl font-semibold text-main focus:outline-none">Agenda tu servicio</h2>
    ...
    {loading && <p className="text-subtle" role="status">Cargando la configuración actual de agendamiento...</p>}

    {!loading && !loadError && <>
      <fieldset className="space-y-3">
        <legend className="text-base font-semibold text-main">1. Elige un servicio</legend>
        {services.length === 0 ? <p className="text-sm text-subtle">Este negocio no tiene servicios disponibles para agendar.</p> : (
      ...
      {selectedService && <section className="space-y-5" aria-labelledby="booking-date-title">
        ...
        {compatibleStaff.length === 0 ? <div className="surface-soft rounded p-4 text-sm text-subtle" role="status">No hay profesionales activos para este servicio. Elige otro servicio o contacta al negocio.</div> : <>
          ...
          <div className="booking-calendar" aria-label="Selecciona una fecha">
             <p id="booking-unavailable-date-hint" className="sr-only">Fecha no disponible para este agendamiento. Elige otro día.</p>
            ...
            <table className="booking-calendar-grid" aria-label={`Fechas de ${monthLabel(calendarMonth || minimumMonth)}`}>
              ...
              <tbody>{calendarWeeks.map((week, weekIndex) => <tr key={`week-${weekIndex}`}>
                {week.map((date, dayIndex) => {
                  ...
                  return <td key={date}><button type="button" className={`booking-calendar-day min-h-11 ${selected ? 'is-selected' : ''} ${unavailable ? 'is-unavailable' : ''}`} aria-pressed={selected} aria-label={`${formatDate(date)}${unavailable ? `. No disponible: ${unavailableReason}` : ''}`} aria-describedby={unavailable ? 'booking-unavailable-date-hint' : undefined} title={unavailableReason || undefined} disabled={unavailable} onClick={() => chooseDate(date)}>{Number(date.slice(-2))}</button></td>;
                })}
              </tr>)}</tbody>
            </table>
          </div>
      ...
      {selectedService && bookingDate && <section ref={timeSectionRef} tabIndex={-1} className="space-y-4 focus:outline-none" aria-labelledby="booking-time-title">
        <div>
          <h3 id="booking-time-title" className="text-base font-semibold text-main">3. Elige una hora</h3>
        ...
        {loadingSlots ? <div className="booking-loading surface-soft rounded p-4" role="status">Buscando horarios disponibles...</div> : slotError ? <div className="error-notice flex flex-wrap items-center gap-3 rounded p-3 text-sm" role="alert"><span>{slotError}</span><button type="button" className="btn-outline px-3 py-1 text-sm" onClick={refreshSlots}>Reintentar</button></div> : slots.length === 0 ? <div className="surface-soft rounded p-4 text-sm"><p className="font-semibold text-main">No quedan horarios disponibles para esta fecha.</p><p className="mt-1 text-subtle">Prueba otro día o deja seleccionado cualquier profesional.</p><button type="button" className="btn-outline mt-3 px-3 py-1 text-sm" onClick={() => setCalendarMonth((month) => shiftMonth(month || minimumMonth, 1))} disabled={calendarMonth >= maximumMonth}>Ver otra fecha</button></div> : <div className="space-y-4">
          <p className="text-sm text-subtle">{availableSlotCount} horarios disponibles. Los horarios marcados no se pueden seleccionar.</p>
          <p id="booking-unavailable-slot-hint" className="sr-only">Horario no disponible para este agendamiento. Elige otra hora.</p>
          {(['Mañana', 'Tarde', 'Noche'] as const).map((period) => slotsByPeriod[period]?.length ? <div key={period}><h4 className="mb-2 text-sm font-semibold text-main">{period}</h4><div className="flex flex-wrap gap-2">{slotsByPeriod[period].map((item) => <button key={item.time} type="button" className={`booking-time min-h-11 ${item.available ? '' : 'is-unavailable'} ${slot?.time === item.time && slot.staffId === item.staffId ? 'is-selected' : ''}`} aria-pressed={item.available && slot?.time === item.time && slot.staffId === item.staffId} aria-label={`${formatTime(item.time)}${item.available ? '' : `. No disponible: ${item.unavailableReason}`}`} aria-describedby={item.available ? undefined : 'booking-unavailable-slot-hint'} title={item.available ? undefined : item.unavailableReason} disabled={!item.available} onClick={() => { if (!item.available) return; setSlot(item); setSubmitError(''); }}>{formatTime(item.time)}</button>)}</div></div> : null)}
        </div>}
      </section>}

      {slot && selectedService && bookingDate && <section className="space-y-4" aria-labelledby="booking-contact-title">
        {selectableProducts.length > 0 && <fieldset className="space-y-3">
          <div><legend className="text-base font-semibold text-main">4. Productos <span className="font-normal text-subtle">(opcional)</span></legend><p className="mt-1 text-sm text-subtle">Agrégalos a tu solicitud. El negocio confirmará disponibilidad y detalles después.</p></div>
          <ul className="booking-product-list" aria-label="Productos disponibles">
            {selectableProducts.map((product) => {
              ...
              return <li key={product.id} className={`booking-product-row ${quantity ? 'is-selected' : ''}`}>
                <BookingProductThumbnail product={product} />
                <div className="booking-product-copy"><h4>{product.name}</h4><p>{description}</p><strong>{formatPrice(product.price)}</strong></div>
                <div className="booking-product-quantity" aria-label={`Cantidad de ${product.name}`}>
                  <button type="button" className="btn-outline" onClick={() => setProductQuantity(product.id, quantity - 1)} disabled={quantity === 0} aria-label={`Reducir cantidad de ${product.name}`}>−</button>
                  <output aria-live="polite" aria-label={`${quantity} ${quantity === 1 ? 'unidad' : 'unidades'} de ${product.name}`}>{quantity}</output>
                  <button type="button" className="btn-outline" onClick={() => setProductQuantity(product.id, quantity + 1)} disabled={quantity >= 10} aria-label={`Aumentar cantidad de ${product.name}`}>+</button>
                </div>
              </li>;
            })}
          </ul>
          {requestedProducts.length > 0 && <p className="booking-product-subtotal" role="status">{requestedProductSubtotal > 0 ? `Subtotal estimado: ${formatPrice(requestedProductSubtotal)}.` : 'Productos añadidos a la solicitud.'} El negocio confirmará el total final.</p>}
        </fieldset>}
        <div><h3 id="booking-contact-title" className="text-base font-semibold text-main">{selectableProducts.length ? '5' : '4'}. Tus datos y confirmación</h3><p className="mt-1 text-sm text-subtle">Usaremos estos datos solo para gestionar tu solicitud.</p></div>
        <div className="booking-confirmation-summary"><span>{selectedService.name}</span><span>{formatDate(bookingDate)} · {formatTime(slot.time)}</span><span>{staffId ? staff.find((member) => member.id === slot.staffId)?.name : 'El negocio asignará a tu profesional.'}</span></div>
        {requestedProducts.length > 0 && <p className="text-sm text-subtle">Productos solicitados: {requestedProducts.map((product) => `${product.name} × ${product.quantity}`).join(', ')}.</p>}
        <div className="grid gap-3 sm:grid-cols-2">
          <label className="space-y-1"><span className="field-label text-sm">Nombre</span><input className="field-input" value={clientName} maxLength={120} onChange={(event) => setClientName(event.target.value)} autoComplete="name" /></label>
          <label className="space-y-1"><span className="field-label text-sm">Celular colombiano</span><input className="field-input" value={clientPhone} maxLength={40} onChange={(event) => setClientPhone(event.target.value)} autoComplete="tel" inputMode="tel" placeholder="300 123 4567" aria-describedby="phone-hint" aria-invalid={Boolean(clientPhone) && (!phoneValid || !phoneLengthValid)} /></label>
          {emailRequested && <label className="space-y-1"><span className="field-label text-sm">Correo electrónico {customerFields?.email === 'required' ? <span aria-hidden="true">*</span> : <span className="font-normal text-subtle">(opcional)</span>}</span><input className="field-input" type="email" value={clientEmail} maxLength={254} onChange={(event) => setClientEmail(event.target.value)} autoComplete="email" aria-required={customerFields?.email === 'required'} aria-invalid={Boolean(emailValue) && !emailValid} /></label>}
          {addressRequested && <label className="space-y-1"><span className="field-label text-sm">Dirección {customerFields?.address === 'required' ? <span aria-hidden="true">*</span> : <span className="font-normal text-subtle">(opcional)</span>}</span><input className="field-input" value={clientAddress} maxLength={240} onChange={(event) => setClientAddress(event.target.value)} autoComplete="street-address" aria-required={customerFields?.address === 'required'} aria-invalid={Boolean(addressValue) && !addressValid} /></label>}
        </div>
        <p id="phone-hint" className="text-xs text-subtle">Ejemplo: 300 123 4567</p>
        <label className="block space-y-1"><span className="field-label text-sm">Nota para el negocio <span className="font-normal text-subtle">(opcional)</span></span><textarea className="field-textarea min-h-24" value={bookingNote} maxLength={BOOKING_NOTE_MAX_LENGTH} onChange={(event) => setBookingNote(event.target.value)} placeholder="Ej.: llegaré unos minutos tarde, prefiero cierto producto o color." aria-describedby="booking-note-hint" aria-invalid={!bookingNoteValid} /><span id="booking-note-hint" className="block text-xs text-subtle">Comparte una solicitud para tu cita. Máximo {BOOKING_NOTE_MAX_LENGTH} caracteres.</span></label>
        {requiresPrivacyAcceptance && <label className="flex items-start gap-3 text-sm text-main"><input className="mt-1 h-4 w-4" type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} /><span>Acepto que el negocio use mis datos para gestionar esta solicitud, conforme al <a className="accent-link font-semibold underline underline-offset-2" href={`${import.meta.env.BASE_URL}tratamiento-de-datos`} target="_blank" rel="noreferrer">tratamiento de datos personales</a> y al <a className="accent-link font-semibold underline underline-offset-2" href={`${import.meta.env.BASE_URL}privacidad`} target="_blank" rel="noreferrer">aviso de privacidad</a>.</span></label>}
        {submitError && <div className="error-notice rounded p-3 text-sm" role="alert">{submitError}</div>}
        <div className="flex flex-wrap items-center gap-3">
          <button type="button" className="btn-primary px-4 py-2" onClick={() => void submit()} disabled={!canConfirm || submitting}>{submitting ? 'Enviando solicitud...' : 'Enviar solicitud de agendamiento'}</button>
          {!canConfirm && !submitting && <button type="button" className="btn-outline px-4 py-2" onClick={showValidationToast} aria-describedby="booking-validation-help">Revisar requisitos</button>}
          <p id="booking-validation-help" className="sr-only">Muestra lo que falta antes de enviar la solicitud.</p>
          {whatsappUrl && <a className="accent-link text-sm font-semibold text-main underline" href={whatsappUrl} target="_blank" rel="noreferrer">¿Prefieres escribir por WhatsApp?</a>}
        </div>
      </section>}
    </>}
  </div>;
