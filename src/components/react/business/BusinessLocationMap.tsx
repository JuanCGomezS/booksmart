...
function RecenterMap({ coordinates }: { coordinates?: BusinessCoordinates }) {
  const map = useMap();
  useEffect(() => {
    map.setView(centerFor(coordinates), coordinates ? LOCATION_ZOOM : DEFAULT_ZOOM);
  }, [coordinates, map]);
  return null;
}
...
function BaseMap({
  ...
  if (tileError)
    return (
      <div>
        <LocationMapFallback coordinates={coordinates} address={address} />
        {retryable && (
  ...
  return (
    <MapContainer
      ...
      zoom={coordinates ? LOCATION_ZOOM : DEFAULT_ZOOM}
      ...
      aria-label={
        interactive
          ? 'Mapa para seleccionar la ubicación exacta del negocio'
...
export function BusinessLocationPicker({
  ...
  value?: BusinessCoordinates;
  onChange: (coordinates: BusinessCoordinates) => void;
}) {
  ...
  const updateFromMap = (coordinates: BusinessCoordinates) => {
  ...
  return (
    <div className="business-location-picker">
      <div className="business-location-picker-copy">
        <strong>Ubicación exacta</strong>
        <span>
          Haz clic en el mapa o ingresa las coordenadas para colocar el punto del negocio.
        </span>
      </div>
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="field-label" htmlFor="business-location-latitude">
          Latitud
          <input
            id="business-location-latitude"
        ...
        <label className="field-label" htmlFor="business-location-longitude">
          Longitud
          <input
            id="business-location-longitude"
            className="field-input mt-1"
            type="number"
            inputMode="decimal"
            min="-180"
      ...
      <p
        id={hintId}
        className={error ? 'error-message mt-2 text-sm' : 'field-hint mt-2'}
        role={error ? 'alert' : undefined}
      >
        {error || 'Latitud: -90 a 90. Longitud: -180 a 180.'}
      </p>
      <BaseMap coordinates={value} interactive>
        <MapClickHandler onChange={updateFromMap} />
      </BaseMap>
      <p className="field-hint">
        OpenStreetMap. Usa el punto de acceso para clientes; la dirección sigue siendo una
