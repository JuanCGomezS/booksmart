  ...
  useEffect(() => {
    ...
    const loadBusinessSummaries = async (user: User, role: UserRole) => {
      ...
      try {
        ...
        if (missingIds.length > 0) setError(`No se publicaron los resúmenes de negocio para: ${missingIds.join(', ')}. Podés administrar los negocios disponibles; solicitá a superadministración crear los resúmenes faltantes.`);
        setSession({ user, role, summaries });
      } catch (cause) {
