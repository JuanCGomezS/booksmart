...
export function formatStaffEnrollmentCode(value: string): string {
  return (
    normalizeStaffEnrollmentCode(value)
      .match(/.{1,5}/g)
      ?.join('-') || ''
  );
}
...
export async function joinBusinessWithCode(enteredCode: string): Promise<void> {
  ...
  if (!user || code.length !== CODE_LENGTH)
  ...
  try {
    await runTransaction(db, async (transaction) => {
      ...
      const name =
        typeof userSnapshot.data().name === 'string' && userSnapshot.data().name.trim()
...
export async function rotateStaffEnrollmentCode(businessId: string): Promise<string> {
  ...
  await runTransaction(db, async (transaction) => {
    ...
    const previousCode =
      controlSnapshot.exists() && typeof controlSnapshot.data().code === 'string'
        ? controlSnapshot.data().code
        : null;
    if (previousCode) transaction.delete(doc(db, 'staffEnrollmentCodes', previousCode));
    transaction.set(doc(db, 'staffEnrollmentCodes', nextCode), {
      businessId,
      createdAt: serverTimestamp(),
    });
...
export async function setEnrollmentStaffStatus(
  ...
  await runTransaction(db, async (transaction) => {
    ...
    if (status === 'active')
      transaction.update(doc(db, 'users', staffId), {
...
export async function retireProfessional(businessId: string, staffId: string): Promise<void> {
  ...
  const cleanupPaths = await runTransaction(db, async (transaction) => {
    ...
    const legacyPhotoPaths =
      typeof profile.photoUrl === 'string'
        ? ['jpg', 'png', 'webp'].map(
            (extension) => `barbers/${businessId}/barbers/${staffId}/image.${extension}`,
          )
        : [];
    const paths = uniqueStoragePaths([
      ...
      ...(Array.isArray(profile.pendingImageCleanupPaths)
        ? profile.pendingImageCleanupPaths.filter(
    ...
    const accountUid =
      typeof profile.accountUid === 'string'
        ? profile.accountUid
  ...
  try {
    ...
    throw new Error(
      'El perfil fue retirado, pero no se pudieron eliminar todas sus fotos. Reintenta el retiro para completar la limpieza.',
    );
  }
}
...
export async function retryRetiredProfessionalPhotoCleanup(
  businessId: string,
  staffId: string,
  initialPaths?: string[],
): Promise<void> {
  ...
  const paths =
    ...
    (await runTransaction(db, async (transaction) => {
      ...
      if (!profileSnapshot.exists() || !isRetiredUnlinkedProfile(profileSnapshot.data())) {
        throw new Error(
          'La limpieza solo está disponible para perfiles retirados y desvinculados.',
        );
      }
      const pendingPaths = profileSnapshot.data().pendingImageCleanupPaths;
      return uniqueStoragePaths(
        Array.isArray(pendingPaths)
          ? pendingPaths.filter((path): path is string => typeof path === 'string')
          : [],
      );
    }));
  ...
  if (Array.isArray(pendingPaths) && pendingPaths.length > 0)
    throw new Error('Aún quedan fotos pendientes de eliminación.');
  await updateDoc(profileRef, {
