...
export const BOOKING_CONFLICT_MESSAGE =
  'Ese horario acaba de ser ocupado. Por favor, elige otra hora.';
export const BOOKING_UNAVAILABLE_MESSAGE =
  'Ese horario ya no está disponible. Por favor, elige otra hora.';
export const BOOKING_PERMISSION_MESSAGE =
  'No tenemos permiso para confirmar la solicitud. Inténtalo nuevamente.';
export const BOOKING_NETWORK_MESSAGE =
  'No pudimos conectar para confirmar la solicitud. Revisa tu conexión e inténtalo nuevamente.';
export const BOOKING_PRECONDITION_MESSAGE =
  'La solicitud cambió antes de confirmarse. Actualiza la página e inténtalo nuevamente.';
export const BOOKING_PHONE_DAY_MESSAGE =
  'Ya tienes una solicitud de agendamiento para este negocio en esa fecha.';
...
function validClientDetails(
  request: ClientBookingRequest,
  customerFields: ReturnType<typeof normalizeBookingCustomerFields>,
): boolean {
  ...
  return {
    ...
    references: {
      ...
      service: (businessId, serviceId) =>
        doc(firestore, 'barbers', businessId, 'services', serviceId),
      staff: (businessId, staffId) => doc(firestore, 'barbers', businessId, 'barbers', staffId),
      appointment: (businessId) =>
        doc(collection(firestore, 'barbers', businessId, 'appointments')),
      lock: (businessId, bookingDate, staffId, intervalId) =>
        doc(
          firestore,
          'barbers',
          businessId,
...
function errorResult(error: unknown): ClientBookingFailure {
  if (
    ...
    ['aborted', 'already-exists', 'conflict'].includes(firestoreErrorCode(error) || '')
  ) {
    if (firestoreErrorCode(error) === 'already-exists')
      return { ok: false, code: 'duplicate_phone_day', message: BOOKING_PHONE_DAY_MESSAGE };
...
export async function createClientBooking<Reference = DocumentReference>(
  ...
  if (!options.adapters && !options.firestore) {
    try {
      const submit = httpsCallable<ClientBookingRequest, { appointmentId: string }>(
        getFunctions(app),
        'createPublicBooking',
      );
  ...
  try {
    return await adapters.transaction.run(async (transaction) => {
      const [businessSnapshot, serviceSnapshot] = await Promise.all([
        transaction.get<PublicBookingConfiguration['business']>(
          references.business(request.businessId),
        ),
        transaction.get<PublicBookingConfiguration['service']>(
          references.service(request.businessId, request.serviceId),
        ),
      ]);
      ...
      if (
        !validClientDetails(request, customerFields) ||
        !validRequestedProducts(request.requestedProducts) ||
        !validBookingNote(request.notes) ||
      ...
      const staffSnapshots = await Promise.all(
        candidateIds.map((staffId) =>
          transaction.get<PublicBookingConfiguration['staff']>(
            references.staff(request.businessId, staffId),
          ),
        ),
      );
      ...
      const candidateLocks = await Promise.all(
        candidates.map(async ({ staffId }) => {
          const refs = intervals.map((interval) =>
            references.lock(request.businessId, request.bookingDate, staffId, interval.id),
          );
          return {
            staffId,
            refs,
            snapshots: await Promise.all(refs.map((reference) => transaction.get(reference))),
          };
        }),
      );
      const selected = candidates.find(({ staffId, staff }) => {
        ...
        return (
          ...
          candidateLocks
            .find((locks) => locks.staffId === staffId)
      ...
      transaction.set(appointmentRef, {
        ...
        ...(customerFields.email !== 'disabled' && request.clientEmail?.trim()
          ? { clientEmail: request.clientEmail.trim() }
          : {}),
        ...(customerFields.address !== 'disabled' && request.clientAddress?.trim()
          ? { clientAddress: request.clientAddress.trim() }
          : {}),
        ...(bookingRequestsAdditionalCustomerData(customerFields)
          ? {
              bookingPrivacyConsent: {
                version: BOOKING_PRIVACY_CONSENT_VERSION,
        ...
        status: 'pending',
        ...(normalizeBookingNote(request.notes)
          ? { notes: normalizeBookingNote(request.notes) }
          : {}),
...
export async function claimUnassignedAppointment(
  ...
  options: {
    firestore?: Firestore;
    now?: Date;
    loadedAgenda?: readonly LoadedAgendaOccupancy[];
  } = {},
  ...
  try {
    return await runTransaction(firestore, async (transaction) => {
      ...
      } as ClaimableAppointment;
      if (
        current.assignmentState !== 'unassigned' ||
        !current.capacityStaffId ||
        !['pending', 'confirmed'].includes(current.status)
      ) {
        return {
          ok: false,
          code: 'unavailable',
          message: 'Esta solicitud ya fue asignada o no está disponible.',
        };
      }
      ...
      if (!isServiceCompatibleWithStaff(service as Service, staffId))
        return { ok: false, code: 'unavailable', message: 'No puedes asumir esta solicitud.' };
      const availability = checkBookingAvailability({
        business,
        service,
        staff,
      ...
      if (!availability.available)
        return {
          ok: false,
          code: 'unavailable',
          message: 'Tu agenda ya no tiene disponibilidad para esta solicitud.',
        };
      ...
      const overlapsLoadedOccupancy = options.loadedAgenda?.some((item) => {
        if (
          item.id === current.id ||
          item.barberId !== staffId ||
          item.bookingDate !== current.bookingDate ||
          !appointmentStatusBlocksAgenda(item.status)
        )
          return false;
        ...
        return (
          ...
          occupiedStart !== null &&
          occupiedEnd !== null &&
          appointmentStart < occupiedEnd &&
          appointmentEnd > occupiedStart
        );
      });
      if (overlapsLoadedOccupancy) {
        return {
          ok: false,
          code: 'unavailable',
          message: 'Tu agenda ya tiene una solicitud en ese horario.',
        };
      }

      const intervalIds = Array.isArray(current.occupiedIntervalIds)
        ? current.occupiedIntervalIds
        : [];
      ...
      const capacityLocks = await Promise.all(
        capacityLockRefs.map((reference) => transaction.get(reference)),
      );
      if (
        capacityLocks.some((lock) => !lock.exists() || lock.data()?.appointmentId !== current.id)
      ) {
      ...
      if (current.capacityStaffId === staffId) {
        transaction.update(appointmentRef, {
          ...
          barberId: staffId,
          capacityStaffId: deleteField(),
          updatedAt: serverTimestamp(),
        });
        return { ok: true };
      }

      const staffLocks = await Promise.all(
        staffLockRefs.map((reference) => transaction.get(reference)),
      );
    ...
    return result.code === 'duplicate_phone_day'
      ...
      : {
          ok: false,
          code: result.code as Exclude<typeof result.code, 'duplicate_phone_day'>,
