...
export const BOOKING_BLOCKING_STATUSES: readonly BookingBlockingAppointmentStatus[] = [
  'pending',
  'confirmed',
];
...
export type BookingAvailability =
  { available: true; endTime: string } | { available: false; reason: BookingAvailabilityReason };
...
export function isPublicBookingAvailable(
  ...
  const cutoff =
    business.bookingEnabledUntil instanceof Date
      ? business.bookingEnabledUntil
      : typeof business.bookingEnabledUntil === 'object' &&
          business.bookingEnabledUntil !== null &&
...
export function bookingSettingsUpdate(
  settings: BookingSettings,
): Record<'config.booking', BookingSettings> {
...
function normalizeClosureRule(value: unknown): ClosureRule | null {
  ...
  if (
    keys.some(
      (key) => !['id', 'kind', 'weekday', 'date', 'startTime', 'endTime', 'reason'].includes(key),
    )
  )
    return null;
  const id =
    typeof rule.id === 'string' && rule.id.trim() && rule.id.trim().length <= 120
      ? rule.id.trim()
      : null;
  ...
  if (
    ...
    (rule.weekday as number) >= 0 &&
    (rule.weekday as number) <= 6
  ) {
...
export function isValidBookingCustomerFields(value: unknown): value is BookingCustomerFields {
  ...
  return (
    ...
    ['email', 'address'].every((field) =>
      ['disabled', 'optional', 'required'].includes(fields[field] as string),
    )
  );
}
...
export function normalizeBookingSettings(value: unknown): BookingSettings {
  ...
  return {
    minimumNoticeMinutes:
      Number.isInteger(configured.minimumNoticeMinutes) &&
      (configured.minimumNoticeMinutes as number) >= 0
        ? (configured.minimumNoticeMinutes as number)
        : DEFAULT_BOOKING_SETTINGS.minimumNoticeMinutes,
    bookingHorizonDays:
      Number.isInteger(configured.bookingHorizonDays) &&
      (configured.bookingHorizonDays as number) >= 1
        ? (configured.bookingHorizonDays as number)
        : DEFAULT_BOOKING_SETTINGS.bookingHorizonDays,
    slotIntervalMinutes:
      Number.isInteger(configured.slotIntervalMinutes) &&
      (configured.slotIntervalMinutes as number) >= 5 &&
      (configured.slotIntervalMinutes as number) <= 120
        ? (configured.slotIntervalMinutes as number)
...
export function isValidBookingSettings(value: unknown): value is BookingSettings {
  ...
  const closureRuleIds = Array.isArray(settings.closureRules)
    ? settings.closureRules.map((rule) => rule?.id)
    : [];
  const exceptionalDates = Array.isArray(settings.exceptionalClosures)
    ? settings.exceptionalClosures.map((closure) => closure?.date)
    : [];
  const validExceptionalClosures =
    Array.isArray(settings.exceptionalClosures) &&
    settings.exceptionalClosures.every(
      (closure) =>
        closure &&
        typeof closure.date === 'string' &&
        Boolean(parseBookingDate(closure.date)) &&
        (closure.reason === undefined ||
          (typeof closure.reason === 'string' &&
            closure.reason.trim().length > 0 &&
            closure.reason.trim().length <= 500)),
    ) &&
  ...
  return (
    ...
    settings.slotIntervalMinutes <= 120 &&
    Array.isArray(settings.closureRules) &&
    settings.closureRules.length <= 100 &&
    new Set(closureRuleIds).size === closureRuleIds.length &&
    settings.closureRules.every((rule) => normalizeClosureRule(rule) !== null) &&
...
export function toPublicBookingSettings(value: unknown): BookingSettings {
  ...
  return {
    ...
    closureRules: [
      ...
      ...settings.exceptionalClosures.map(({ date }) => ({
        id: `legacy-${date}`,
        kind: 'date' as const,
        date,
      })),
...
export function isBookingDateFullyClosed(
  settings: Pick<BookingSettings, 'closureRules' | 'exceptionalClosures'>,
  bookingDate: string,
): boolean {
  ...
  return settings.closureRules.some(
    (rule) =>
      !rule.startTime &&
      !rule.endTime &&
...
function closureRuleOverlapsBooking(
  rule: ClosureRule,
  bookingDate: string,
  weekday: number,
  start: number,
  ...
  if (
    ...
    (rule.kind === 'date' && rule.date !== bookingDate)
  )
    return false;
...
export function getBookingLockPath(
  businessId: string,
  bookingDate: string,
  staffId: string,
  intervalId: string,
...
export function hasBookingLockConflict(
  requiredIntervalIds: readonly string[],
  occupiedIntervalIds: ReadonlySet<string>,
): boolean {
...
export function getBogotaDateTime(bookingDate: string, time: string): Date | null {
  ...
  return new Date(
    Date.UTC(date.year, date.month - 1, date.day, 0, minutes + BOGOTA_UTC_OFFSET_MINUTES),
  );
}
...
export function appointmentStatusBlocksAgenda(
  status: string,
): status is BookingBlockingAppointmentStatus {
  ...
  return Object.fromEntries(
    Array.from({ length: 7 }, (_, day) => {
      ...
      const configuredStart =
        typeof source.start === 'string' && timeToMinutes(source.start) !== null
          ? source.start
          : CLOSED_DAY.start;
      const configuredEnd =
        typeof source.end === 'string' && timeToMinutes(source.end) !== null
          ? source.end
          : CLOSED_DAY.end;
      ...
      const breaks = Array.isArray(source.breaks)
        ? source.breaks.flatMap((item) => {
            ...
            return typeof range.start === 'string' &&
              ...
              validScheduleRange(range.start, range.end) &&
              range.start >= start &&
              range.end <= end
...
export function getStaffSchedule(
  staff: BarberStaff,
  ...
  return Object.fromEntries(
    Array.from({ length: 7 }, (_, day) => {
      ...
      if (!businessDay?.enabled || !validScheduleRange(businessDay.open, businessDay.close))
      ...
      if (!staffDay)
        return [
          day,
          { enabled: true, start: businessDay.open, end: businessDay.close, breaks: [] },
        ];
      if (!staffDay.enabled || !validScheduleRange(staffDay.start, staffDay.end))
        return [day, CLOSED_DAY];

      const start =
        timeToMinutes(staffDay.start)! > timeToMinutes(businessDay.open)!
          ? staffDay.start
          : businessDay.open;
      const end =
        timeToMinutes(staffDay.end)! < timeToMinutes(businessDay.close)!
          ? staffDay.end
          : businessDay.close;
      ...
      return [
        day,
        {
          ...
          breaks: Array.isArray(staffDay.breaks) ? staffDay.breaks : [],
        },
      ];
    }),
  ) as WeeklySchedule;
...
export function isServiceCompatibleWithStaff(
  service: Pick<Service, 'staffIds'>,
  staffId: string,
): boolean {
...
export function checkBookingAvailability(input: BookingAvailabilityInput): BookingAvailability {
  ...
  if (!isPublicBookingAvailable(business, input.now))
    return { available: false, reason: 'business_inactive' };
  ...
  if (!isServiceCompatibleWithStaff(service as Service, input.staffId))
    return { available: false, reason: 'service_incompatible' };
  ...
  if (
    !requestedAt ||
    requestedAt.getTime() < now.getTime() + settings.minimumNoticeMinutes * 60_000 ||
    horizonDays < 0 ||
    horizonDays > settings.bookingHorizonDays
  ) {
    return { available: false, reason: 'outside_booking_window' };
  }
  if (settings.exceptionalClosures.some((closure) => closure.date === bookingDate))
    return { available: false, reason: 'exceptional_closure' };
  ...
  if (
    settings.closureRules.some((rule) =>
      closureRuleOverlapsBooking(rule, bookingDate, weekday, start, end),
    )
  ) {
  ...
  if (
    schedule.breaks.some((breakRange) => {
  ...
  if (
    !endTime ||
    !getBookingLockIntervals(startTime, duration, buffer, settings.slotIntervalMinutes)
  ) {
