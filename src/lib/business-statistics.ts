...
export async function getBusinessStatistics(
  ...
  const statuses: BusinessStatistics['statuses'] = {
    pending: 0,
    confirmed: 0,
    done: 0,
    cancelled: 0,
  ...
  snapshot.forEach((document) => {
    ...
    if (typeof item.bookingDate === 'string')
      byDate.set(item.bookingDate, (byDate.get(item.bookingDate) || 0) + 1);
    const id = typeof item.serviceId === 'string' ? item.serviceId : 'unknown';
    const entry = byService.get(id) || {
      serviceId: id,
      name: typeof item.serviceName === 'string' ? item.serviceName : 'Servicio sin nombre',
      total: 0,
    };
    entry.total += 1;
  ...
  const series =
    days <= 31
      ? Array.from({ length: days + 1 }, (_, index) => {
          ...
          return { label: day.slice(5), total: byDate.get(day) || 0 };
        })
      : [...byDate.entries()]
